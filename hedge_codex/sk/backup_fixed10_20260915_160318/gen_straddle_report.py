...
<p class="note">平价关系参考：<a href="https://prd-web.optionseducation.org/advancedconcepts/put-call-parity">OIC Put/Call Parity</a>。本报告展示历史条件模拟，不把非同步价差当作可执行收益。</p>''','notes'))
    ...
    html=f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SKHY 1 Call＋1 Put 策略回测报告</title><style>{css}</style></head><body><div class="wrap"><h1>SKHY 1 Call＋1 Put 策略回测报告</h1><p class="sub">资金口径已更新 · 实际保费本金固定1组／整组满仓滚动 · {start}～{end}（44个交易日）<br>保持原跌15% / 涨8%滚动规则 · 生成于 {now} 北京时间</p><nav><a href="#capital">本金与收益</a><a href="#capital-chart">仓位净值</a><a href="#full-trades">满仓明细</a><a href="#conclusion">旧现金基准</a><a href="#comparison">净值比较</a><a href="#decomposition">差额拆解</a><a href="#trades">逐轮明细</a><a href="#paired">同轮配对</a><a href="#matrix">阈值矩阵</a><a href="#cash">现金利息</a></nav>{''.join(parts)}</div></body></html>'''
    out.write_text(html)
    print('Report:',out)
