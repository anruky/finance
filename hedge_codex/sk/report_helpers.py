...
def chart(a,b):
 vals=[[r.get('unit_nav',r['equity']/x['initial'])*100 for r in x['curve']] for x in (a,b)]
 ...
 return ''.join(out)+'</svg><p>金：策略1（10 Call＋10 Put）；蓝：策略2（100股＋2 Put）。单位净值初始为100，入金按当时净值增加份额；包含现金及未平仓资产市值，入金本身不产生收益。</p>'
