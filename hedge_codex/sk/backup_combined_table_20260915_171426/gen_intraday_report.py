...
<li>日终净值保留收盘估值，以衡量持仓期间回撤；缺日终价格时使用最近已知价与内在价值较大者并标记。期末未平仓只估值，不假装10:00已卖出。</li>
 ...
 for i,r in enumerate(rows,1):
  x,z=r['strategy1'],r['strategy2'];label=r['entry']+' → '+r['exit'];status=('同轮／同分钟' if r['same_execution_times'] else '同轮／成交分钟不同') if r['matched'] else '路径不同／仅本策略成交'
  times='<br>'.join(name+'：'+t['entry_time']+' → '+t['exit_time'] for name,t in [('策略1',x),('策略2',z)] if t)
  costrows.append([i,label,times,f"${r['strike']:g} / {r['expiry']}",status,fmt(x['call_cost']/10 if x else None),fmt(x['put_cost']/10 if x else None),fmt(r['s1_unit_cost']),fmt(z['stock_cost'] if z else None),fmt(z['put_cost'] if z else None),fmt(z['cost'] if z else None)])
  profitrows.append([i,label,times,'<br>'.join(name+'：'+t['reason'] for name,t in [('策略1',x),('策略2',z)] if t),fmt(r['s1_unit_pnl']),fmt(z['stock_pnl'] if z else None),fmt(z['put_pnl'] if z else None),fmt(z['pnl'] if z else None),fmt(r['difference']),pct(100*x['pnl']/x['cost']) if x else '—',pct(100*z['pnl']/z['cost']) if z else '—'])
 parts.append(section('逐轮成本：1 Call＋1 Put vs 100股＋2 Put',table(['轮次','入场日 → 退出日','实际入场 → 退出时间','行权价／到期','配对状态','1 Call成本','1 Put成本','策略1单组总成本','100股含费买入','2 Put成本','策略2总成本'],costrows)+'<p>策略2股票列现在是本轮真实模拟买入支出，已含买股费用；每轮退出另扣卖股费用。相同入场日、退出日及Put合约合并为一轮，成交分钟允许不同，并分别列出两策略实际时间。收益差额包含成交时间差的影响，不能理解为同一时点的纯结构差；日期或合约不同仍单列。</p>','costs'))
 parts.append(section('逐轮收益：1 Call＋1 Put vs 100股＋2 Put',table(['轮次','入场日 → 退出日','实际入场 → 退出时间','退出方式','策略1单组净收益','策略2股票净收益','策略2 Put净收益','策略2总收益','策略1单组－策略2','策略1轮收益率','策略2轮收益率'],profitrows),'profits'))
