...
<p class="note">说明：SKHY 上市仅 {meta['n_days']} 个交易日，MA20 样本偏短、信号仅供参考。现价跌破 MA20 说明短期偏弱。</p>
</div>
...
① 开头空档：首日 {meta['entry_date']} 无期权数据（上市首日），股票从 ${meta['entry_price']:.2f} 裸涨到第一轮入场 ${_first_entry_spot:.2f}，计 <span class="{pc(_gap_head)}">${_gap_head:+,.0f}</span>；<br>
...
<p style="font-size:13px;color:var(--text);line-height:1.9;">原因：成本 &gt;11% 的 {len(_skip_rounds)} 个轮次合计净亏 {money(sum(_skip_rounds))}（{', '.join(f'{v:+,.0f}' for v in _skip_rounds)}），跳过它们就把这部分亏损避开了。注意：这里是"事后跳过"的对比口径（其他轮次不受影响），实盘若轮空导致重新进场的时点错位，收益会有差异。SKHY 仅 31 个交易日，样本小，仅供参考。</p>
</div>

<div class="card">
<h2>亏损轮的特征分析</h2>
<div class="callout-warn">
<strong>结论：亏损 / 小赚的轮次，几乎全是「波动不够大」的轮次。</strong>买 put 赚的是"波动"的钱——只有实际涨跌幅超过权利金成本（约 10%），put 的赔付才能覆盖保费。波动小的轮次（无论涨还是跌），权利金白付，就是亏损轮。
</div>
<ul style="font-size:13px;color:var(--text);padding-left:20px;line-height:1.9;">
<li><strong>亏损轮只有两类</strong>：①「上涨再平衡」轮（涨了触发追高，但追高后没继续涨，put 变虚值 + 重新付保费）；②「到期」小跌轮（跌了但跌得不够狠，put 赔付 &lt; 保费，股票又小亏）。</li>
<li><strong>大赚轮只有两类</strong>：①「下跌止盈」轮（跌 15%+ 触发熔断，2:1 对冲下 put 赔付是股票亏损的 2 倍）；②「极端大涨」轮（如涨 23%，股票涨 + 追高前 put 也涨，双击）。</li>
<li><strong>入场时 ATR 越高，越容易亏</strong>：亏损轮入场 ATR% 均值约 10.6%，明显高于大赚轮的 8.5%。原因——高波动时 IV 高、权利金贵、成本占比大，后续要有更大波动才能回本。</li>
<li><strong>入场 RSI 偏高（50 附近）易亏，偏低（46 附近）易赚</strong>：RSI 中性偏高时追高买入，回调概率大；RSI 偏低（接近超卖）时买入，反弹概率大。</li>
</ul>
<p class="note">注意：SKHY 仅 31 个交易日、共 {best['n_rounds']} 轮，样本量很小，上述特征是"这一段行情"的观察，统计意义有限，仅供参考，不可外推为普适规律。</p>
</div>
