...
def main():
 ...
 parts=[]
 parts.append(section('10:00–15:59执行窗口下的账户结果','<div class="callout-gold"><strong>主情景采用10:00–15:59最早共同成交分钟。</strong>下列结果仍可能包含缺价跳单、延期及到期例外，不代表实际10:00报价下可完整执行的策略收益。请结合交易轮数与数据检查阅读。</div>'+table(['项目',a['name'],b['name']],[
...
<li>日终净值保留收盘估值，以衡量持仓期间回撤；缺日终价格时使用最近已知价与内在价值较大者并标记。期末未平仓只估值，不假装10:00已卖出。</li>
 ...
 def signed(v,percent=False):
  text=f'{v:+.2f}%' if percent else f'${v:+,.2f}'
  return '<span class="'+('c-red' if v>=0 else 'c-green')+'">'+text+'</span>'
 def stack(v1,v2):return '<div class="round-leg">'+v1+'</div><div class="round-leg">'+v2+'</div>'
 def leg_price(t,leg):
  if not t:return '—'
  if leg=='call' and not t['pairs']:return '<span class="c-gray">—（未持有）</span>'
  bar=d['minutes'].get(t[leg],{}).get(t['entry_date'],{}).get(t['entry_time'])
  return money(bar['o']) if bar else '—'
 combined=[]
 for r in reversed(rows):
  x,z=r['strategy1'],r['strategy2'];pair=[x,z]
  def col(fn):return stack(*[fn(t,j) if t else '—' for j,t in enumerate(pair)])
  entry=col(lambda t,j:t['entry_date']+'<br><span class="c-gray">'+t['entry_time']+'</span>')
  exit=col(lambda t,j:t['exit_date']+'<br><span class="c-gray">'+t['exit_time']+'</span>')
  mode=col(lambda t,j:t['reason'])
  structure=stack('<span class="c-gold">策略1 · 1 Call＋1 Put</span>','<span class="c-red">策略2 · 100股＋2 Put</span>')
  spot_in=col(lambda t,j:money(t['entry_spot']))
  spot_out=col(lambda t,j:money(t['exit_spot']))
  move=col(lambda t,j:signed(100*(t['exit_spot']/t['entry_spot']-1),True))
  strike=money(r['strike'])+'<br><small class="c-gray">到期 '+r['expiry']+'</small>'
  put=col(lambda t,j:leg_price(t,'put'));call=col(lambda t,j:leg_price(t,'call'))
  cost=stack(('<span class="c-green">'+money(x['cost']/10)+'</span>') if x else '—',('<span class="c-green">'+money(z['put_cost'])+'</span><br><small class="c-gray">股票 '+money(z['stock_cost'])+'<br>合计 '+money(z['cost'])+'</small>') if z else '—')
  income=stack(('<span class="c-red">'+money(x['income']/10)+'</span>') if x else '—',('<span class="c-red">'+money(z['put_income'])+'</span><br><small class="c-gray">股票回收 '+money(z['stock_income'])+'</small>') if z else '—')
  profit=stack((signed(x['pnl']/10)+'<br><small class="c-gray">收益率 '+pct(100*x['pnl']/x['cost'])+'</small>') if x else '—',(signed(z['pnl'])+'<br><small class="c-gray">股票损益 '+money(z['stock_pnl'])+'<br>收益率 '+pct(100*z['pnl']/z['cost'])+'</small>') if z else '—')
  delta='<div class="round-difference">单组策略1－策略2：'+(signed(r['difference']) if r['difference'] is not None else '不可配对')+'</div>'
  combined.append([entry,exit,mode,structure,spot_in,spot_out,move,strike,put,call,cost,income,profit+delta])
 header=['入场日','出场日','方式','结构','入场spot','出场spot','波动','行权价','put价','call价','成本','收入','利润']
 combined_html=table(header,combined).replace('<table>','<table class="round-details">',1)
 parts.append(section('两策略逐轮明细：跌15%／涨8%', '<span id="profits"></span><p class="note">参考原报告的逐轮明细格式，最新周期在前；每个周期上行为策略1（1 Call＋1 Put），下行为策略2（100股＋2 Put）。日期相同则合并，实际成交分钟分别列出。策略1账户总览仍为10组。</p>'+combined_html+'<p class="note">Put/Call价为入场分钟原始首笔价，不含滑点和佣金。成本与收入主数字沿用参考表的期权口径：策略1为Call＋Put，策略2为2 Put；策略2股票买入成本、总投入、股票回收额和股票净损益在小字中补充。利润包含该策略全部资产损益和费用，期末持有行的收入为估值而非已兑现现金。波动为各自出入场股价变化。收益差额含成交分钟不同的影响；策略2收益率分母为股票＋Put总投入。</p>','costs'))
 ...
 css+='\n.round-details td {vertical-align:top;} .round-details .round-leg {min-height:88px;padding:8px 0;} .round-details .round-leg + .round-leg {border-top:1px dashed var(--border);} .round-details .round-difference {font-size:11px;border-top:1px solid var(--border);padding-top:6px;} .round-details small {font-size:11px;line-height:1.5;} .round-details tbody tr:nth-child(even) {background:rgba(255,255,255,.018);}'
