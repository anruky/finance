...
def run_straddle(data,calls,cfg=Config(),initial_capital=None,all_in=False,fixed_pairs=1):
    stock=data['stock'];initial=stock[0][4]*100*(1+cfg.reserve) if initial_capital is None else initial_capital
    assert initial>0
    assert isinstance(fixed_pairs,int) and not isinstance(fixed_pairs,bool) and fixed_pairs>0
            ...
            if candidate is None:skipped['no_put']+=1
                ...
                if ct not in calls['listed'].get(prev[0],[])or pc is None or previous['v']<cfg.min_volume or pc+.05<max(prev[4]-con['strike_price'],0):
                    ...
                    if cp is None or pp is None:skipped['no_fill']+=1
                        ...
                        pairs=int((cash+1e-9)/(cc+ppc)) if all_in else fixed_pairs
                        if pairs<1 or pairs*(cc+ppc)>cash+1e-8:skipped['cash']+=1
                        else:
    ...
    return dict(config=asdict(cfg),name='1 Call＋1 Put＋现金',initial=initial,all_in=all_in,fixed_pairs=fixed_pairs,**m,fees=fees,premium=premium,
