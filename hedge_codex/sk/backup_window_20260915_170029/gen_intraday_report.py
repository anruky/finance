...
def main():
 ...
 parts=[]
 parts.append(section('10:00交易规则下的账户结果','<div class="callout-gold"><strong>严格10:00分钟成交样本存在明显缺口。</strong>下列结果包含缺价跳单、延期及到期例外，不代表实际10:00报价下可完整执行的策略收益。请结合交易轮数与数据检查阅读。</div>'+table(['项目',a['name'],b['name']],[
...
<li>每日美东时间10:00检查股票相对本轮入场价是否下跌15%或上涨8%；触发则卖出旧组合并买入新组合。到期日也在10:00滚动，不等收盘才正常平仓。</li>
<li>10:00决策使用刚结束的09:59分钟收盘价；按这个可见价格重新选择近似平值合约。到期目标仍为2天、范围1–7天，行权价与当时股价偏离不超过3%。先按期限距离、再按行权价距离排序。</li>
<li>两策略使用同一候选池：Call和Put在09:30–09:59各累计至少成交10张。成交量筛选仅用决策前数据，替代旧版前日成交量筛选；此共同池为配对比较而设。先确定合约，之后若缺10:00成交，不用其他合约或全天价格替换。</li>
<li>成交代理：股票、Call、Put均使用10:00–10:01这一分钟的首笔成交价，买卖加减每边2%滑点（期权至少每股$0.01）及每张$0.65佣金；股票每次买入和卖出各5bps。各资产首笔可能发生在不同秒，分钟数据不能保证同步盘口成交。</li>
<li>策略1每轮10 Call＋10 Put，资金不足完整10组则跳过；策略2每轮先卖100股和旧Put，再同时买100股和2 Put，资金不足整个组合则跳过。首次入场前两账户均持现金。</li>
<li>平仓缺必要期权的10:00数据则整个旧组合延期，不伪造单腿成交。到期仍无法在10:00平仓时，以收盘内在价值现金等价结算并处置股票，属于明确列示的到期例外，并非10:00成交。</li>
<li>日终净值保留收盘估值，以衡量持仓期间回撤；缺日终价格时使用最近已知价与内在价值较大者并标记。期末未平仓只估值，不假装10:00已卖出。</li>
 ...
 for i,r in enumerate(rows,1):
  x,z=r['strategy1'],r['strategy2'];label=r['entry']+' 10:00 → '+r['exit']+' '+(x or z)['exit_time'];status='同轮可比' if r['matched'] else '路径不同／仅本策略成交'
 ...
 quality=[]
 for k,label in [('no_candidate','10:00前无合格候选'),('no_entry_minute','开仓缺10:00成交'),('no_exit_minute','平仓缺10:00成交／延期'),('cash','资金不足／跳过')]:quality.append([label,a['skipped'][k],b['skipped'][k]])
 ...
 html=f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SKHY 美东10:00两策略回测</title><style>{css}</style></head><body><div class="wrap"><h1>SKHY 开盘半小时后交易：两策略对比</h1><p class="sub">策略1账户：10 Call＋10 Put；逐轮展示1 Call＋1 Put<br>策略2：每轮卖出再买入100股＋2 Put<br>每天10:00判断与执行 · 下跌15%／上涨8% · 数据至09-11 · 更新 {now}</p><nav><a href="#overview">结果</a><a href="#rules">10:00规则</a><a href="#costs">逐轮成本</a><a href="#profits">逐轮收益</a><a href="#quality">数据检查</a></nav>{''.join(parts)}</div></body></html>'''
