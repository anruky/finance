...
def main():
 ...
 for i,(x,z) in enumerate(zip(a['trades'],b['trades']),1):
  ...
  rounds.append(dict(round=i,entry=x['entry_date'],exit=x['exit_date'],expiry=x['expiry'],strike=x['strike'],reason=x['reason'],mark_only=x['mark_only'],
   s1_call_cost=x['call_cost'],s1_put_cost=x['put_cost'],s1_cost=x['cost'],s2_stock_value=stock_value,s2_put_cost=z['cost'],s2_capital=stock_value+z['cost'],
   s1_pnl=x['pnl'],s2_stock_pnl=stock_pnl,s2_put_pnl=z['pnl'],s2_pnl=stock_pnl+z['pnl'],difference=x['pnl']-stock_pnl-z['pnl'],
 ...
 assert abs(a['pnl']-sum(t['s1_pnl'] for t in rounds))<1e-6
 result=dict(strategy1=a,strategy2=b,rounds=rounds,strategy2_outside_round_pnl=outside,manifest=calls['manifest'])
...
<p><strong>逐轮成本：</strong>策略1列本轮两种期权的实际含费支出；策略2列100股在本轮入场时的市值＋两张Put含费支出，作为轮初资产占用参考。股票市值并非本轮新增支付，不能把每轮股票市值相加作为累计投入。</p>
<p><strong>逐轮收益：</strong>策略1是20张期权净损益；策略2是该轮100股价格变动损益＋两张Put净损益。股票首次买入费用放在轮次外对账，避免重复扣费。期末仍持有的最后一轮只计市值。各轮收益率仅按各自轮初资产成本计算，不能相加当账户收益率。</p>''','rules'))
 parts.append(section('同本金账户净值',chart(a,b),'equity'))
 parts.append(section('每轮成本对比',table(['轮次','入场 → 退出','行权价／到期','策略1 Call支出','策略1 Put支出','策略1 合计支出','策略2 股票市值','策略2 Put支出','策略2 资产占用'],[
 ...
 parts.append(section('每轮收益对比',table(['轮次','入场 → 退出','退出方式','策略1 净收益','策略2 股票损益','策略2 Put损益','策略2 合计收益','策略1－策略2','策略1 轮收益率','策略2 轮收益率'],[
 ...
 parts.append(section('逐轮与全期账本对账',table(['项目','策略1','策略2'],[
 ['期权轮次内合计净收益',money(sum(t['s1_pnl'] for t in rounds)),money(sum(t['s2_pnl'] for t in rounds))],
 ['轮次外净收益（含首次股票费用）',money(0),money(outside)],['全期净收益',money(a['pnl']),money(b['pnl'])]])+f'<p>策略2于07-13买入100股，首轮期权07-15才开仓；这段股票收益扣除首次买股费用后为 {money(outside)}。它单列在轮次外，因此逐轮收益合计与全期收益不同。所有交易行、逐日现金及资产市值均保存在结果JSON。</p>','reconcile'))
 sens=[]
 for slip in (0,.02,.05):
...
<p><a href="skhy_strategy_comparison_results.json">完整两策略结果与逐轮数据</a> · <a href="skhy_simulation_report.html">原100股＋2Put报告及波动相关因素研究</a></p>''','notes'))
 ...
 html=f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SKHY 两策略逐轮成本与收益对比</title><style>{css}</style></head><body><div class="wrap"><h1>SKHY 两策略成本与收益对比</h1><p class="sub">策略1：每轮10 Call＋10 Put ｜ 策略2：100股＋每轮2 Put<br>共同下跌15%／上涨8%滚动 · 2026-07-13～09-11 · 更新于 {now}</p><nav><a href="#overview">总览</a><a href="#costs">逐轮成本</a><a href="#profits">逐轮收益</a><a href="#reconcile">账本对账</a></nav>{''.join(parts)}</div></body></html>'''
