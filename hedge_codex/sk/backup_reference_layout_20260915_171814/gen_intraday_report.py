...
<li>日终净值保留收盘估值，以衡量持仓期间回撤；缺日终价格时使用最近已知价与内在价值较大者并标记。期末未平仓只估值，不假装10:00已卖出。</li>
 ...
 for i,r in enumerate(rows,1):
  ...
  times='<br>'.join(name+'：'+t['entry_time']+' → '+t['exit_time'] for name,t in [('策略1',x),('策略2',z)] if t)
  reasons='<br>'.join(name+'：'+t['reason'] for name,t in [('策略1',x),('策略2',z)] if t)
  c1=fmt(r['s1_unit_cost'])+('<br><small>Call '+money(x['call_cost']/10)+'<br>Put '+money(x['put_cost']/10)+'</small>' if x else '')
  c2=fmt(z['cost'] if z else None)+('<br><small>股票 '+money(z['stock_cost'])+'<br>Put '+money(z['put_cost'])+'</small>' if z else '')
  p1=fmt(r['s1_unit_pnl'])+('<br><small>Call '+money(x['call_pnl']/10)+'<br>Put '+money(x['put_pnl']/10)+'</small>' if x else '')
  p2=fmt(z['pnl'] if z else None)+('<br><small>股票 '+money(z['stock_pnl'])+'<br>Put '+money(z['put_pnl'])+'</small>' if z else '')
  combined.append([i,r['entry']+' → '+r['exit']+'<br><small>'+times+'</small>',f"${r['strike']:g}<br>{r['expiry']}",c1,p1,pct(100*x['pnl']/x['cost']) if x else '—',c2,p2,pct(100*z['pnl']/z['cost']) if z else '—',fmt(r['difference']),reasons])
 headers=['轮次','交易日期／实际时间','行权价／到期','总成本','净收益','轮收益率','总成本','净收益','轮收益率','收益差额<br>策略1－策略2','退出方式']
 grouped='<tr><th colspan="3">交易信息</th><th colspan="3">策略1：1 Call＋1 Put</th><th colspan="3">策略2：100股＋2 Put</th><th colspan="2">比较与退出</th></tr>'
 combined_html=table(headers,combined).replace('<thead>','<thead>'+grouped,1)
 parts.append(section('逐轮成本与收益：1 Call＋1 Put vs 100股＋2 Put','<span id="profits"></span>'+combined_html+'<p>每个成本、收益单元格上方为合计，下方为各腿明细。策略1逐轮按单组展示，账户总览仍为10组；策略2成本含本轮股票买入费用，净收益另扣股票卖出及期权退出费用。相同入场日、退出日与Put合约合并一轮，分别保留实际成交分钟；收益差额包含成交时间差的影响。轮收益率＝本轮净收益÷本轮总成本，不能相加作为账户收益率。</p>','costs'))
