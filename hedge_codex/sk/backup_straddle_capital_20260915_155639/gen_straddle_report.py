...
def main():
    ...
    result=dict(manifest=calls['manifest'],base=st,original=original,benchmark=bh,paired=paired,
    ...
    parts=[]
    parts.append(section('核心结果',f'''<div class="callout-gold"><strong>1张Call＋1张Put的基准模拟全期收益为 {money(st['pnl'])}，资金收益率 {st['return_pct']:.2f}%。</strong><br>比原100股＋2张Put全期少 {money(original['pnl']-st['pnl'])}，但不能把这个差额全部理解为跨式更差：原策略在首轮期权开仓前已获得股票收益。</div>{cards}
...
<p class="note">严格沿用旧报告 {start}～{end} 的44个交易日做可比试验，没有把新Call与更晚股票日期混用。Call补拉时间为 {calls['manifest']['fetched_at_utc']}。</p>''','model'))
    parts.append(section('与原策略和基准比较',table(['策略','全期盈亏','资金收益率','最大回撤','收益/回撤','新增期盈亏','新增期收益率'],[[r['name'],money(r['pnl']),f'{r["return_pct"]:.2f}%',f'{r["mdd_pct"]:.2f}%',ratio(r['pnl_mdd']),money(r['holdout']['pnl']),f'{r["holdout"]["return_pct"]:.2f}%']for r in (st,original,bh)])+'<p class="note">新增区间为08-25～09-11，继承08-24持仓与现金。资金收益率按各区间期初实际净值计算，故同时列示美元盈亏；不采用只除以期权保费的高杠杆收益率口径。</p>'+chart([st,original,bh]),'comparison'))
...
<p class="note">平价关系参考：<a href="https://prd-web.optionseducation.org/advancedconcepts/put-call-parity">OIC Put/Call Parity</a>。本报告展示历史条件模拟，不把非同步价差当作可执行收益。</p>''','notes'))
    ...
    html=f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>SKHY 1 Call＋1 Put 策略回测报告</title><style>{css}</style></head><body><div class="wrap"><h1>SKHY 1 Call＋1 Put 策略回测报告</h1><p class="sub">独立新报告 · 同档同到期跨式＋剩余现金 · {start}～{end}（44个交易日）<br>保持原跌15% / 涨8%滚动规则 · 生成于 {now} 北京时间</p><nav><a href="#conclusion">核心结果</a><a href="#comparison">净值比较</a><a href="#decomposition">差额拆解</a><a href="#trades">逐轮明细</a><a href="#paired">同轮配对</a><a href="#matrix">阈值矩阵</a><a href="#cash">现金利息</a></nav>{''.join(parts)}</div></body></html>'''
    out.write_text(html)
    print('Report:',out)
