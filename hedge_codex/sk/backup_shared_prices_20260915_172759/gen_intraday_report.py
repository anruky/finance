...
<li>日终净值保留收盘估值，以衡量持仓期间回撤；缺日终价格时使用最近已知价与内在价值较大者并标记。期末未平仓只估值，不假装10:00已卖出。</li>
 ...
 for r in reversed(rows):
  x,z=r['strategy1'],r['strategy2']
  def compare(fn):return paired(fn(x) if x else '—',fn(z) if z else '—')
  cost1=money(x['cost']/10) if x else '—';income1=money(x['income']/10) if x else '—'
  profit1=signed(x['pnl']/10) if x else '—'
  cost2=money(z['put_cost']) if z else '—';income2=money(z['put_income']) if z else '—'
  profit2=signed(z['pnl']) if z else '—'
  combined.append([r['entry'],r['exit'],compare(lambda t:t['entry_time']+' → '+t['exit_time']),compare(lambda t:t['reason']),
   compare(lambda t:money(t['entry_spot'])),compare(lambda t:money(t['exit_spot'])),compare(lambda t:f"{100*(t['exit_spot']/t['entry_spot']-1):+.2f}%"),money(r['strike']),r['expiry'],
   raw(x,'call'),raw(x,'put'),cost1,income1,profit1,pct(100*x['pnl']/x['cost']) if x else '—',
   raw(z,'put'),money(z['stock_cost']) if z else '—',cost2,money(z['cost']) if z else '—',income2,money(z['stock_income']) if z else '—',profit2,pct(100*z['pnl']/z['cost']) if z else '—',signed(r['difference']) if r['difference'] is not None else '—'])
 headers=['入场日','出场日','实际时间（入→出）','方式','入场spot','出场spot','波动','行权价','到期日',
  'Call价','Put价','成本','收入','利润','收益率',
  'Put价','股票买入','Put成本','总成本','Put收入','股票回收','利润','收益率','利润差①－②']
 grouped='<tr><th colspan="9">周期信息（①策略1／②策略2）</th><th colspan="6">① 策略1：1 Call＋1 Put</th><th colspan="8">② 策略2：100股＋2 Put</th><th>对比</th></tr>'
 combined_html=table(headers,combined).replace('<table>','<table class="round-details">',1).replace('<thead>','<thead>'+grouped,1)
 parts.append(section('两策略逐轮明细：每个周期一条记录','<span id="profits"></span><p class="note">最新周期在前，一行对应一个周期。日期与合约只列一次，两策略成本、收入和利润横向并列；实际时间、股价或退出方式不同时，以①／②在同一单元格注明。表格可横向滚动。</p>'+combined_html+'<p class="note">①逐轮为1 Call＋1 Put，账户仍为10组；②为100股＋2 Put。期权价是入场分钟原始首笔价。①成本/收入为两腿含费收支；②单列Put收支、股票买入和回收，总成本为股票＋Put含费支出。两策略利润均含全部资产损益和费用。期末持有的收入为估值。收益差额包含成交时间差影响；收益率分母为各自本轮总成本。</p>','costs'))
 ...
 css+='\n.round-details td {vertical-align:middle;} .round-details tbody tr:nth-child(even) {background:rgba(255,255,255,.018);} .round-details th {position:static;} .round-details td:nth-child(10),.round-details td:nth-child(16),.round-details td:nth-child(24) {border-left:2px solid var(--border);}'
