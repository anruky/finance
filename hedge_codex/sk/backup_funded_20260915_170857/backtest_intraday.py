...
def run_intraday(data,strategy,initial=22852.50,slip=.02,down=15,up=8,min_volume=10,window=5):
 assert 0<=window<=59
 ...
 return dict(name='策略1：10 Call＋10 Put' if strategy==1 else '策略2：100股＋2 Put',strategy=strategy,initial=initial,**m,trades=trades,curve=curve,skipped=skips,flags=flags,events=events,reconciliation=residual,
  config=dict(down=down,up=up,slip=slip,min_morning_volume=min_volume,decision='09:59 completed minute close at 10:00',execution='earliest common minute 10:00 through window end; reentry strictly after exit minute',window_minutes=window,stock_bps=5,commission=.65))
