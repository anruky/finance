...
def write_markdown(payload,out):
 ...
 for c in payload['cycles']:
  ...
  lines+=['']
 content='\n'.join(lines)
 section='## gav 复盘解读 · 报告1\n\n以下为 brd_gav.md 中的个人复盘原文，尚未按统一退出规则回测验证。“总收益5%”的分母及9/4“15%左右”的指标尚未明确，不据此生成交易信号。\n\n周期7/31 ：整个区间股票都是负的， 最优策略是在总收益在5% 的时候平仓\n\n8/7 - 整个区间股价都是正的， 最优策略是 持有到期。 或者股价涨20% 后平仓\n\n8/14 - 股价先正后负， 最优策略是在周三，股票跌但是有收益的时候平仓\n\n8/21 - 股价先正后负， 最优策略是股价负，收益正的时候平仓\n\n8/28 - 股价一直小正，到最后大正。  最优策略是一直到期平仓\n\n9/4 - 股价一直大正， 最后策略是15% 左右平仓\n\n'
 index=content.index('## ')
 content=content[:index]+section+content[index:]
 (out/'skhy_weekly_cycle_report.md').write_text(content)
