...
OUT_HTML = "/Users/gavinz/git/finance/hedge/sandisk/sndk_v10_backtest_report.html"
...
def load_3fri():
    data = json.load(open(os.path.join(DATA, "SNDK_options_3fri.json")))
    return data, {d["date"]: d for d in data}


def load_stock():
    stock = json.load(open(os.path.join(DATA, "SNDK_stock.json")))
...
def run_v10(day_map, closes, bars, stock, stock_entry, stock_exit, move_pct, target, num_puts=2, up_pct=None):
...
def main():
    ...
    json.dump({"bh": bh, "results": results}, open("/tmp/sndk_v10_results.json", "w"), indent=2, default=str)
...
<title>SNDK 对冲 v10 —— 周期 × 熔断线对比</title>
<style>
...
<body>
<div class="wrap">
<h1>SNDK 对冲 v10 <span style="color:var(--muted);font-size:15px;">（周期 × 熔断线二维对比）</span></h1>
