#!/usr/bin/env python3
"""
SNDK (SanDisk) 多周五到期日期权拉取 (未来 3 个周五)
=============================================================
与 hedge/sk/data_puller_3fri.py 同逻辑，仅 TICKER 改为 SNDK。
SanDisk（闪迪）为 NAND 闪存/存储公司，Nasdaq 上市，2025-02 从西部数据分拆。

对每个交易日拉"未来 3 个周五"到期日的 near-ATM 期权链，
用于"7天轮动"回测(选 dte 最接近 7 天的那个周五到期日)。

数据源: Polygon.io
输出: /Users/gavinz/git/finance/data/SNDK_options_3fri.json
"""
import json
import os
import math
