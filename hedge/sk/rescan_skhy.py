#!/usr/bin/env python3
"""
SKHY 对冲策略 —— 全参数重新模拟（临时程序，不动现有 final 程序/报告）
=====================================================================
在最新数据（截至 09-11，44 个交易日）上，重新扫描四维参数空间：
...
输出临时报告：skhy_rescan_report.html
"""
import json
import os
...
OUT = os.path.join(os.path.dirname(__file__), "skhy_rescan_report.html")
...
<body>
<div class="wrap">
<h1>SKHY（SK海力士 ADR）对冲策略 —— 全参数重扫</h1>
<p class="sub">临时报告 · 数据 {stock[0][0]} ~ {stock[-1][0]}（{len(stock)} 交易日） · 生成于 {datetime.now(ET).strftime('%Y-%m-%d %H:%M')}（美东 ET）</p>
